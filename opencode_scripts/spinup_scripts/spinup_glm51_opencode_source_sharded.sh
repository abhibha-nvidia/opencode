#!/bin/bash
# =========================================================================
#  run_glm51_opencode_source_sharded.sh   --   RUN WITH `bash`, NOT sbatch.
#
#  Same as run_glm51_opencode_sharded.sh, but runs the opencode harness FROM
#  SOURCE (via Bun) instead of the OOB precompiled binary. Edits under
#  packages/opencode/src/** take effect immediately -- no rebuild needed.
#
#  How: OPENCODE_BIN points at the `opencode-src` wrapper (next to this file),
#  which execs `bun run packages/opencode/src/index.ts "$@"`. The driver still
#  invokes `${OPENCODE_BIN} run ...` exactly as before, so nothing else changes.
#
#  One-time setup (already done once; redo on a fresh clone):
#    export BUN_INSTALL=<repo>/bun
#    curl -fsSL https://bun.sh/install | bash -s bun-v1.3.14
#    cd <repo> && <repo>/bun/bin/bun install   # needs node-gyp on PATH
#
...
#    NUM_SHARDS=4 bash run_glm51_opencode_source_sharded.sh
# =========================================================================
set -euo pipefail
...
JOB_PREFIX="${JOB_PREFIX:-glm51_ocsrc}"
...
echo "============================================"
echo "  GLM-5.1 opencode rollout [FROM SOURCE]$( ((RESUMING==1)) && echo '  [RESUME]' )"
